...
public class TokenSigner
{
    ...
    public string Sign(string data)
    {
        ...
        var signerInfo = new BCSignerInfo(
            ...
            GetSignedAttributes(data),
            new AlgorithmIdentifier(new DerObjectIdentifier("1.2.840.113549.1.1.11")),
            new DerOctetString(SignWithToken(session, key, data)),
            null);

        // Fix SignedData creation
        var signedData = new BCSignedData(
            new DerSet(new AlgorithmIdentifier(new DerObjectIdentifier("2.16.840.1.101.3.4.2.1"))), // digestAlgorithms
            contentInfo,                                                                             // contentInfo
            new DerSet(new DerSequence(new DerOctetString(cert.RawData))),                         // certificates
            null,                                                                                   // crls
    ...
    private DerSet GetSignedAttributes(string data)
    {
    ...
    private byte[] SignWithToken(ISession session, IObjectHandle key, string data)
    {
        var mechanism = session.Factories.MechanismFactory.Create(CKM.CKM_SHA256_RSA_PKCS);
        return session.Sign(mechanism, key, Encoding.UTF8.GetBytes(data));
    }
